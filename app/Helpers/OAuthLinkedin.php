...
class OAuthLinkedin extends OAuth {
    ...
    public function getAuthUrl (): array {
        ...
        $url = static::URL_AUTHORIZE . '?redirect_uri=' . urlencode( $this->urlCallback ) . '&client_id=' . $this->consumerKey . '&scope=' . urlencode( 'r_liteprofile r_emailaddress' ) . '&response_type=code' . '&state=' . $state;
    ...
    protected function getToken ( ?string $code, ?string $state ): ?array {
        ...
        $response = $this->client->send( new Request( 'POST', static::URL_ACCESS_TOKEN, [
    ...
    protected function baseText (): string {
        return "GET&" . urlencode( static::URL_AUTHORIZE ) . "&" . urlencode( "redirect_uri=" . urlencode( $this->urlCallback ) . "&" . "client_id=" . $this->consumerKey . "&" . "scope=" . urlencode( 'r_liteprofile r_emailaddress' ) . "&" . "response_type=code" );
    }
